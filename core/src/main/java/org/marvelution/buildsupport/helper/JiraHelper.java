        ...
        implements Closeable
{
    ...
    {
        this(baseUri,
                new CommonsHttpTransport(builder().credentials(some(credentials))
                        .build(), baseUri));
    }
            ...
            HttpTransport httpTransport)
    {
        ...
        this.baseUri = norm.getPath()
                .endsWith("/") ? norm : URI.create(norm + "/");
    ...
    {
        URI searchURI = UriBuilder.fromUri(SEARCH_URL)
                .build();

        byte[] bytes = encode(request);

        try (SimpleHttpResponse response = httpTransport.post(resolveLink(searchURI),
